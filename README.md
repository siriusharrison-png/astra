...
| I want to explain Astra to someone else (what it is / does / how to build their own) | `SHARE.md` | Self-contained, project-neutral explainer — no install path; covers the value difference and the aspects that matter when building your own |
...
| I need to identify or name a design style | `patterns.md` Design Style Identification Pattern | Name the style + lock 3-5 defining signals before extracting tokens (qualitative → quantitative) |
| My spec reads vague / amateurish and I want every line testable | `patterns.md` Verifiable Specification Pattern | Four-question conversion (quantify/enumerate/boundary/feedback) + trigger-word blacklist turns vague lines into acceptance criteria |
